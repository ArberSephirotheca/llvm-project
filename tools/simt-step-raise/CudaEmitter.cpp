...
struct CudaEmitter {
    ...
    explicit CudaEmitter(llvm::raw_ostream &os) : os(os) {}
    ...
    std::string emitExpr(Operation *op) {
        ...
        if (auto c = dyn_cast<arith::ConstantIndexOp>(op)) {
    ...
    LogicalResult emitSwitch(simt::dialect::SwitchOp sw) {
        ...
        if (caseValues.size() + 1 != numBlocks)
            llvm::report_fatal_error("CudaEmitter: switch case_values size mismatch");
        auto defaultIndexAttr = sw.getDefaultIndexAttr();
        if (!defaultIndexAttr)
            llvm::report_fatal_error("CudaEmitter: switch missing default_index");
        int64_t defaultIndex = defaultIndexAttr.getInt();
        if (defaultIndex < 0 ||
            static_cast<std::size_t>(defaultIndex) >= numBlocks)
            llvm::report_fatal_error("CudaEmitter: switch default_index out of range");

        std::vector<bool> caseFallthrough;
        caseFallthrough.reserve(numBlocks);
        for (auto &blk : region) {
            if (blk.empty())
                llvm::report_fatal_error("CudaEmitter: switch case missing yield");
            auto y = dyn_cast<simt::dialect::YieldOp>(blk.back());
            if (!y)
                llvm::report_fatal_error("CudaEmitter: switch case missing yield");
            auto attr = y->getAttrOfType<mlir::BoolAttr>("fallthrough");
            if (!attr)
                llvm::report_fatal_error("CudaEmitter: switch missing fallthrough attr");
        ...
        if (caseValueIndex != caseValues.size())
            llvm::report_fatal_error("CudaEmitter: switch case_values mapping mismatch");
    ...
    LogicalResult emitOp(Operation *op) {
        ...
        if (auto did = dyn_cast<simt::dialect::DispatchThreadIdOp>(op)) {
            ...
            emitIndent();
            os << emitType(did.getResult().getType()) << " " << tmp << " = tid;\n";
            return success();
        }
        if (isa<arith::AddIOp, arith::RemSIOp, arith::CmpIOp,
            ...
            emitIndent();
            os << emitType(op->getResult(0).getType()) << " " << tmp << " = "
               << emitExpr(op) << ";\n";
            return success();
        }
        if (auto br = dyn_cast<simt::dialect::BreakOp>(op)) {
        ...
        if (auto call = dyn_cast<func::CallOp>(op)) {
            if (call.getNumResults() > 1)
                llvm::report_fatal_error("CudaEmitter: call with multiple results");
        ...
        if (auto ret = dyn_cast<func::ReturnOp>(op)) {
            ...
            llvm::report_fatal_error("CudaEmitter: multiple return values");
        }
...
static LogicalResult emitHelperFunction(func::FuncOp func,
    ...
    if (results.size() > 1)
        llvm::report_fatal_error("CudaEmitter: helper multiple results");
    ...
    os << "static __device__ __forceinline__ " << retType << " "
       << func.getName() << "(";
    bool first = true;
...
LogicalResult emitModuleAsCuda(ModuleOp module, llvm::raw_ostream &os) {
    func::FuncOp func;
    ...
    os << "static __device__ __forceinline__ int simt_lane_id(int tid) {\n";
    os << "  return tid % SIMT_SUBGROUP_WIDTH;\n";
    os << "}\n";
    os << "static __device__ __forceinline__ int simt_subgroup_id(int tid) {\n";
    os << "  return tid / SIMT_SUBGROUP_WIDTH;\n";
    os << "}\n";
    os << "static __device__ __forceinline__ unsigned simt_subgroup_mask() {\n";
    os << "  unsigned lane = static_cast<unsigned>(threadIdx.x & 31);\n";
    os << "  unsigned width = SIMT_SUBGROUP_WIDTH;\n";
    os << "  if (width >= 32) return 0xffffffffu;\n";
    os << "  unsigned base = (lane / width) * width;\n";
    os << "  unsigned mask = ((1u << width) - 1u) << base;\n";
    os << "  unsigned blockMask = (blockDim.x >= 32) ? 0xffffffffu : ((1u << blockDim.x) - 1u);\n";
    ...
    for (auto arg : func.getArguments()) {
        ...
        if (mlir::isa<simt::dialect::ResourceType>(arg.getType())) {
            os << "int* buf" << arg.getArgNumber();
        } else {
            os << CudaEmitter(os).emitType(arg.getType()) << " arg" << arg.getArgNumber();
        }
    }
    os << ") {\n";
    os << "  int tid = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);\n";
