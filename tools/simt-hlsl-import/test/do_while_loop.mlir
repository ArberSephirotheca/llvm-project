...
// MLIR:     %[[IS_BREAK:.*]] = "arith.cmpi"(%[[CONT_STAGE1]]#1, %[[THREE]]) <{predicate = 0 : i64}> : (i32, i32) -> i1
// MLIR:     %[[BREAK_STAGE1:.*]]:4 = "simt_step.if"(%[[IS_BREAK]]) ({
// MLIR:       %[[BREAK_TRUE:.*]] = "arith.constant"() <{value = true}> : () -> i1
// MLIR:       "simt_step.yield"(%[[BREAK_TRUE]], %[[CONT_STAGE1]]#1, %[[CONT_STAGE1]]#2, %[[FALSE2]]) : (i1, i32, i32, i1) -> ()
// MLIR:     }, {
// MLIR:       %[[BREAK_FALSE:.*]] = "arith.constant"() <{value = false}> : () -> i1
// MLIR:       "simt_step.yield"(%[[BREAK_FALSE]], %[[CONT_STAGE1]]#1, %[[CONT_STAGE1]]#2, %{{.*}}) : (i1, i32, i32, i1) -> ()
// MLIR:     }) : (i1) -> (i1, i32, i32, i1)
...
// MLIR:       "simt_step.yield"() : () -> ()
// MLIR:     }) {simt.normalized.loop_terminators} : (i1) -> ()
// MLIR:     %[[ACC_NEXT:.*]] = "arith.addi"(%[[CONT_STAGE1]]#2, %[[CONT_STAGE1]]#1) {{.*}}
// MLIR:     %[[INC_MAIN:.*]] = "arith.constant"() <{value = 1 : i32}> : () -> i32
// MLIR:     %[[I_NEXT:.*]] = "arith.addi"(%[[CONT_STAGE1]]#1, %[[INC_MAIN]]) {{.*}}
